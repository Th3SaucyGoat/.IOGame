...
namespace KS.Reactor.Client.Unity
{
    ...
    public class ksPhysicsSettings : MonoBehaviour
    {
        ...
        // Data from versions prior to 0.9.13
#pragma warning disable 649 // Disable unassigned member warning
        [SerializeField][HideInInspector]
        private float m_sleepThreshold;
        [SerializeField][HideInInspector]
        private float m_contactOffset;
        [SerializeField][HideInInspector]
        private float m_maxLinearSpeed;
        [SerializeField][HideInInspector]
        private float m_maxAngularSpeed;
        [SerializeField][HideInInspector]
        private float m_maxDepenetrationSpeed;
        [SerializeField][HideInInspector]
...
#pragma warning restore 649
...
#if UNITY_EDITOR
        /// <summary>
        /// Called when the script is deserialized or script values change in the editor. Moves default entity physics
        /// data from the component (where it was stored in versions prior to 0.9.13) to
        /// <see cref="DefaultEntityPhysics"/> if this component was last saved in a version prior to 0.9.13.
        /// </summary>
        private void OnValidate()
        {
            if (m_velocityIterations != 0)
            {
                // Prior to 0.9.13 we stored DefaultEntityPhysics data in the component. Move the data to
                // DefaultEntityPhysics.
                UpgradePhysicsSettings();
                // Set velocity iteration to 0 (a value that used to be invalid) to indicate we moved the data from the
                // component into DefaultEntityPhysics.
        ...
        /// <summary>
        /// Moves default entity physics data from the component where it was stored in versions prior to 0.9.13 into
        /// <see cref="DefaultEntityPhysics"/>
        /// </summary>
        private void UpgradePhysicsSettings()
        {
            DefaultEntityPhysics.SleepThreshold = m_sleepThreshold;
            DefaultEntityPhysics.ContactOffset = m_contactOffset;
            DefaultEntityPhysics.MaxLinearSpeed = m_maxLinearSpeed;
            DefaultEntityPhysics.MaxAngularSpeed = m_maxAngularSpeed;
            DefaultEntityPhysics.MaxDepenetrationSpeed = m_maxDepenetrationSpeed;
            DefaultEntityPhysics.PositionIterations = m_positionIterations;
            DefaultEntityPhysics.VelocityIterations = m_velocityIterations;
        }
#endif
    }
}
