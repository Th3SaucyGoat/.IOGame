    ...
    public class ksConnectionHUD : MonoBehaviour
    {
        ...
        public void DrawGUI(int windowId)
        {
            ...
            else if (m_room != null && m_room.IsConnected)
            {
                ...
                GUILayout.Label($"Connected to {m_room.Info.Host}:{m_room.Info.Port}");
            ...
            else
            {
                ...
                {
                    ksRoomInfo roomInfo = FindObjectOfType<ksRoomType>().GetRoomInfo();
                    roomInfo.Host = Host;
                    roomInfo.Port = Port;
...
#if UNITY_WEBGL && !UNITY_EDITOR
            ksWSConnection.Config.Secure = roomInfo.Host != "localhost" && roomInfo.Host == "127.0.0.1";
        ...
        protected virtual void OnConnect(ksBaseRoom.ConnectError status, uint customStatus)
        {
            if (status != ksBaseRoom.ConnectError.CONNECTED)
            {
                m_errorMessage = "Error: " +  status + " (" + customStatus + ")";
                ksLog.Error(this, m_errorMessage);
            }
        }

        /// <summary>Cleanup room on disconnect. Override to disable or modify</summary>
        /// <param name="status">Connection status.</param>
        protected virtual void OnDisconnect(ksBaseRoom.DisconnectError status)
        {
            m_room.CleanUp();
