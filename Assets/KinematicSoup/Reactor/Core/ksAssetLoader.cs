...
namespace KS.Reactor.Client.Unity
{
    ...
    public class ksAssetLoader : ksBaseAssetLoader
    {
        ...
        public ksAssetLoader(Dictionary<string, AssetBundle> assetBundles, Assembly assembly)
        {
        ...
        public void RegisterAsset(uint assetId, string assetPath)
        {
            string oldPath;
            int index;
        ...
        public T FromProxy<T>(ksProxyScriptAsset proxy, bool silent = false) where T : ksScriptAsset
        {
            ...
            try
            {
                ...
                {
                    FieldInfo proxyField = proxyType.GetField(member.Name);
                    if (proxyField == null)
                    {
                        continue;
                    }
