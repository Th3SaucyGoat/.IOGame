...
namespace KS.Reactor.Client.Unity.Editor
{
    ...
    public class ksServerScriptCompiler
    {
        ...
        /// <summary>
        /// Moves the built assemblies to the <see cref="ksPaths.BuildDir"/>. Restarts local servers.
        /// </summary>
        private void MoveAssembliesAndRestartServers()
        {
            try
            {
                ...
                ksPathUtils.CopyFiles(
                    ksPaths.BuildDir + "../../KSServerRuntime",
                    ksPaths.BuildDir,
                    new string[] { ".dll", ".pdb" },
        ...
        private ProcessStartInfo BuildProcessStartInfo(string args, string toolPath)
        {
            ...
            buildIStartInfo.WorkingDirectory = ksPaths.ProjectRoot + Path.DirectorySeparatorChar;
            ...
            m_thread = new Thread(delegate()
            {
                try
                {
                    using (Process process = new Process())
                    {
                        ...
                        process.StartInfo = BuildProcessStartInfo(args, ksReactorConfig.Instance.Server.BuildToolPath);
        ...
        private void OSXBuild(bool rebuild)
        {
            m_thread = new Thread(delegate()
            {
                try
                {
                    string command = "build --target:Build --configuration:" + GetConfigurationName() + " \"" + ksPaths.ServerRuntimeSolution + "\"";
                    Process mdtoolProcess = Process.Start(BuildProcessStartInfo(command, ksReactorConfig.Instance.Server.BuildToolPath));
                    string buildOutput = command + "\n" + mdtoolProcess.StandardOutput.ReadToEnd();
                    m_result = MacOutputParser(buildOutput);
        ...
        /// <summary>Parses Windows build output.</summary>
        /// <param name="buildOutput">stdout from build process.</param>
        /// <returns>Compile result</returns>
        private CompileResults WindowsOutputParser(string buildOutput)
        {
            if (GetStatusCount(buildOutput, "failed") != 0)
            {
                ksLog.Error(this, "KSServerRuntime build failed " + buildOutput);
                return CompileResults.FAILURE;
            }
            if (GetStatusCount(buildOutput, "succeeded") != 0)
            {
                ksLog.Info(this, "KSServerRuntime build succeeded " + buildOutput);
                return CompileResults.SUCCESS;
            }
            if (GetStatusCount(buildOutput, "skipped") != 0 || GetStatusCount(buildOutput, "up-to-date") != 0)
            {
                ksLog.Info(this, "KSServerRuntime up-to-date " + buildOutput);
                return CompileResults.UP_TO_DATE;
            }
            ksLog.Error(this, "KSServerRuntime build failed " + buildOutput);
            return CompileResults.FAILURE;
        }

        /// <summary>Parses Mac build output.</summary>
        /// <param name="buildOutput">stdout from build process.</param>
        /// <returns>Compile result</returns>
        private CompileResults MacOutputParser(string buildOutput)
        {
            bool buildFailed = buildOutput.Contains("Build FAILED");
            bool proxyScriptsGenerated = buildOutput.Contains("Proxy scripts generated");
            if (!buildFailed)
            {
                ksLog.Info(this, "KSServerRuntime build succeeded\n" + buildOutput);
                if (proxyScriptsGenerated)
                {
                    return CompileResults.SUCCESS;
                }
                return CompileResults.UP_TO_DATE;
            ...
            {
                ksLog.Error(this, "KSServerRuntime build failed\n" + buildOutput);
                return CompileResults.FAILURE;
            }
        }
