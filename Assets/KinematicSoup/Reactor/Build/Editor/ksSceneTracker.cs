    ...
    public class ksSceneTracker : ScriptableObject
    {
        ...
        public List<string> GetScenesReferencingModifiedAssets()
        {
            ...
            foreach (KeyValuePair<string, long> pair in m_assetTimes)
            {
                ...
                bool wasDeleted = !File.Exists(ksPaths.ProjectRoot + "/" + assetPath);
                if (wasDeleted)
                {
                ...
                foreach (string scenePath in referencedScenePaths)
                {
                    ...
                    // Add scenes that were referencing deleted assets
                    if (wasDeleted)
                    {
                    ...
                    // Add the scene to the list if the asset was modified since the last time it was tracked.
                    long fileTime = File.GetLastWriteTimeUtc(ksPaths.ProjectRoot + "/" + assetPath).Ticks;
