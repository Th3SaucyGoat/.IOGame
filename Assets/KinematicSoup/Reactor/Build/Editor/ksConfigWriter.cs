...
namespace KS.Reactor.Client.Unity.Editor
{
    ...
    public class ksConfigWriter
    {
        ...
        /// <summary>Summary of scene and room configs written to file.</summary>
        public ksJSON SceneSummary
        {
            get { return m_sceneSummary; }
        }
        private ksJSON m_sceneSummary = null;
            ...
            bool regenProjectFiles = false)
        {
            ...
            try
            {
                ...
                if (!Directory.Exists(ksPaths.BuildDir))
                {
                    ksPathUtils.Create(ksPaths.BuildDir, true);
                }
        ...
        private SceneBuildResults WriteSceneConfig(Scene scene)
        {
            ...
            // Add the scene and its rooms to the scene summary
            m_sceneSummary[scene.name] = new ksJSON(ksJSON.Types.ARRAY);
            foreach (KeyValuePair<string, ksJSON> pair in sceneConfig["rooms"].Fields)
            {
                m_sceneSummary[scene.name].Add(pair.Key);
            }
        ...
        private void DeleteAllSceneConfigs()
        {
            try
            {
                DirectoryInfo dirInfo = new DirectoryInfo(ksPaths.BuildDir);
        ...
        /// <summary>Expand a scene in the hierarchy window.</summary>
        /// <param name="scene">Unity scene</param>
        /// <param name="expand">Expand if true.</param>
        private static void ExpandSceneHierarchy(Scene scene, bool expand)
        {
           foreach (var window in Resources.FindObjectsOfTypeAll<SearchableEditorWindow>())
           {
                ...
                object expandTarget = window;

                // Some versions of unity have the expand method we want to call inside a SceneHierarchy object of 
                // the window and on others the expand method exists on the SceneHierarchyWindow objects itself. 
                FieldInfo sceneHeirarchyInfo = window.GetType().GetField(
                    "m_SceneHierarchy",
                    BindingFlags.NonPublic | BindingFlags.Instance
                );
                if (sceneHeirarchyInfo != null)
                {
                    expandTarget = sceneHeirarchyInfo.GetValue(window);
                ...
                MethodInfo method = expandTarget.GetType().GetMethod(
                    "ExpandTreeViewItem",
                    BindingFlags.NonPublic | BindingFlags.Instance, 
                    null,
                ...
                FieldInfo field = scene.GetType().GetField(
                    "m_Handle",
                    BindingFlags.NonPublic | BindingFlags.Instance
                );
                if (field == null)
                ...
                method.Invoke(expandTarget, new System.Object[] { sceneHandle, expand });
           }
        }
        ...
        public ksJSON CreateSceneJson()
        {
            ...
            foreach (GameObject instance in instanceList)
            {
                ...
                CharacterController prefabCharacterController = prefabEntity == null ? null : prefabEntity.GetComponent<CharacterController>();
                UpdateProgressBar(null, "Writing character controller configs for " + entity.name);
            ...
            for (int i = 0; i < entityObjects.Count; i += 2)
            {
                ...
                ksJSON jsonScripts = CreateScripts<ksProxyEntityScript>(instance, prefab, true);
                if (jsonScripts != null)
                {
        ...
        private bool CreateRoomType(ksJSON jsonSettings, GameObject gameObject)
        {
            ...
            if (prefabType == null || prefabType.NetworkSyncRate != roomType.NetworkSyncRate)
            {
                json["communication rate"] = ksRoom.SIMULATION_RATE / roomType.NetworkSyncRate;
            }
        ...
        private ksJSON CreateScripts<ProxyScript>(GameObject gameObject, GameObject prefab = null, bool isInstance = false) where ProxyScript : ksProxyScript
        {
            ksJSON json = new ksJSON(ksJSON.Types.ARRAY);
                ...
                if (prefabComponents != null)
                {
                    ...
                    if (prefabComponent != null)
                    {
                        string prefabScriptName = GetScriptName<ProxyScript>(
                            ...
                            prefabComponent);
                        isPrefabComponent = prefabScriptName != "";
                    }
                    ...
                    while (prefabIndex < prefabComponents.Length && prefabComponents[prefabIndex] != prefabComponent)
                    {
        ...
        private ksJSON CreateScript<ProxyScript>(string scriptName, Component component, bool isPrefabComponent) where ProxyScript : Component
        {
            ...
            SerializedProperty defaults = null;
            if (!isPrefabComponent)
            {
                jsonScript["name"] = scriptName;
                // Create a temporary default script to get default script values.
                defaults = new SerializedObject(GetScript(component.GetType()) as ProxyScript).GetIterator();
            }
            ...
            while (iter.NextVisible(enterChildren))
            {
                if (defaults != null)
                {
                    defaults.NextVisible(enterChildren);
                }
                enterChildren = false;
                ...
                if (field != null)
                {
                    if (defaults != null)
                    {
                        // If the field value is the same as the default value, we don't write it.
                        ksJSON defaultJson = CreateField(defaults);
                        if (field.Equals(defaultJson))
                        {
                            continue;
                        }
                    }
        ...
        private ksJSON CreateColliderScript(string scriptName, GameObject gameObject, Component component, bool isPrefabComponent, bool isInstance)
        {
            ...
            if (isInstance && isPrefabComponent)
            {
                ...
                ksEntityComponent prefabEntity = PrefabUtility.GetCorrespondingObjectFromSource(entity);
                Component prefabComponent = PrefabUtility.GetCorrespondingObjectFromSource(component);
        ...
        private ksJSON CreateRigidBodyScript(string scriptName, GameObject gameObject, Component component, bool isPrefabComponent)
        {
            ...
            ksEntityComponent defaultEntity = PrefabUtility.GetCorrespondingObjectFromSource(entity);
            ...
            if (defaultPhysics == null)
            {
                defaultPhysics = new ksEntityPhysicsSettings(
                    -1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1, -1, ksRigidBodyTypes.DEFAULT);
            }
            ...
            if (physics.RigidBodyType != defaultPhysics.RigidBodyType)
                jsonParams["rigid body type"] = (int)physics.RigidBodyType;
        ...
        private ksJSON CreateCharacterControllerScript(string scriptName, Component component, bool isPrefabComponent)
        {
            ...
            ksCollisionFilterAsset collisionFilter = entity.GetCollisionFilterAsset(collider);
            ...
            if (isPrefabComponent)
            {
                ...
                ksEntityComponent prefabEntity = PrefabUtility.GetCorrespondingObjectFromSource(entity);
                ksCollisionFilterAsset prefabCollisionFilter = prefabEntity.GetCollisionFilterAsset(prefabCollider);
        ...
        private ksJSON GetOverrideProperty(string propertyPath, SerializedObject so, SerializedObject dso)
        {
            ...
            SerializedProperty dsp = dso.FindProperty(propertyPath);
            if (dsp == null)
            {
                return null;
            }
            ksJSON defaultValue = CreateField(dsp);
            ...
            ksEntityPhysicsSettings defaults = null)
        {
            ...
            if (data.RigidBodyType != ksRigidBodyTypes.DEFAULT && (defaults == null || data.RigidBodyType != defaults.RigidBodyType))
                json["rigid body type"] = (int)data.RigidBodyType;
            ...
            if (index == -1)
            {
                ...
                if (index == -1)
                {
                    ...
                    else
                    {
                        ...
                        data.Id = entity.GetShapeId(collider);
